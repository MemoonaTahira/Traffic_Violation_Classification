...
wget.download("https://github.com/MemoonaTahira/Traffic_Violation_Classification/releases/download/latest/cleaned_traffic_violations.csv",
              out="cleaned_traffic_violations.csv")
...
df['violation_type'] = df['violation_type'].apply(
    lambda x: x if (x != "sero" and x != "esero") else "ero")
...
def create_model_pipeline(use_cat_OHE=False):
    ...
    categorical_columns = x_train_full.select_dtypes(
        include=["object_", "category"]).columns.tolist()
    boolean_columns = x_train_full.select_dtypes(
    ...
    numerical_transformer = Pipeline(
        steps=[("numerical_imputer", numerical_imputer),
               ("scaler", numerical_robust_scaler), ],
        verbose=True
    )
    categorical_imputer = SimpleImputer(strategy='most_frequent')
    categorical_OHE_encoder = OneHotEncoder(
        max_categories=30, drop=None, sparse=True, handle_unknown="ignore")
    ...
    model_pipeline = Pipeline(
        steps=[
            ...
            ('feature_selection', SelectKBest(
                score_func=mutual_info_classif, k=100)),
...
tuned_model_classification = XGBClassifier(objective='multi:softmax', learning_rate=0.03,
                                           max_depth=20, n_estimators=200, eval_metric=balanced_accuracy_score, n_jobs=-1, random_state=42)
